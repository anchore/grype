...
// any rpm arch other than "src". Entries with no architecture qualifier (older databases
// or providers that don't distinguish source from binary) pass through unchanged, so
// existing upstream-indirected matching against source-granularity advisories keeps
// working.
//
...
func SourceOrUnspecifiedArch() vulnerability.Criteria {
	return search.ByFunc(func(vuln vulnerability.Vulnerability) (bool, string, error) {
		for _, q := range vuln.PackageQualifiers {
			...
			a := ra.Arch()
			if a == "" || a == architecture.ArchSource {
				continue
			}
