...
//nolint:funlen
func (m *Matcher) Match(provider vulnerability.Provider, p pkg.Package) ([]match.Match, []match.IgnoredMatch, error) {
...
func findEUSMatches(provider vulnerability.Provider, p pkg.Package, upstreamMatcher match.MatcherType) ([]match.Match, []match.IgnoredMatch, error) {
	...
	disclosures, err := provider.FindDisclosures(
	...
	resolutions, err := provider.FindResolutions(
		search.ByPackageName(p.Name),
...
func selectDisclosureRange(d distro.Distro) []distro.Distro {
	...
	newD, err := distro.New(d.Type, strconv.Itoa(f[0]), d.IDLike...)
	if err != nil {
		return []distro.Distro{d} // if we cannot create a new distro, we will just return the original
	}
...
func selectResolutionRange(d distro.Distro) []distro.Distro {
	if d.Designator == "" {
		return []distro.Distro{d}
	}
	...
	for i := 2; i <= minor; i += 2 {
		...
		newD, err := distro.New(d.Type, ver, d.IDLike...)
		if err != nil {
			continue // if we cannot create a new distro, we will just skip it
		}
		newD.Designator = d.Designator // ensure we keep the designator
...
func isEUSContext(d *distro.Distro) bool {
	...
	return strings.ToLower(d.Designator) == "eus"
}

type collection struct {
	matchPrototype
	ids             *strset.Set
	disclosuresByID map[string][]vulnerability.Disclosure
	resolutionsByID map[string][]vulnerability.Resolution
}
...
func newCollection(prototype matchPrototype, ds ...vulnerability.Disclosure) *collection {
	c := &collection{
		matchPrototype:  prototype,
		ids:             strset.New(),
		disclosuresByID: make(map[string][]vulnerability.Disclosure),
		resolutionsByID: make(map[string][]vulnerability.Resolution),
	}
	c.AddDisclosures(ds...)
	return c
}

func (c *collection) AddDisclosures(ds ...vulnerability.Disclosure) {
	for _, d := range ds {
		if d.ID == "" {
		...
		if existing, ok := c.disclosuresByID[d.ID]; ok {
			...
			c.disclosuresByID[d.ID] = []vulnerability.Disclosure{d}
		}
	}
}

func (c *collection) AddResolutions(rs ...vulnerability.Resolution) {
	for _, r := range rs {
		if r.ID == "" {
		...
		if existing, ok := c.resolutionsByID[r.ID]; ok {
			...
			c.resolutionsByID[r.ID] = []vulnerability.Resolution{r}
		}
	}
}
