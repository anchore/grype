...
// apkMatch builds a minimal match for the identity-based dedup tests. The
// vulnerability is identified by id, with any aliases recorded as
// RelatedVulnerabilities (as a CGA record would carry its CVE alias).
func apkMatch(id string, aliases ...string) match.Match {
	var related []vulnerability.Reference
	for _, a := range aliases {
		related = append(related, vulnerability.Reference{ID: a})
	}
	return match.Match{
		Vulnerability: vulnerability.Vulnerability{
			Reference:              vulnerability.Reference{ID: id},
			RelatedVulnerabilities: related,
		},
	}
}

// Test_deduplicateMatches verifies that CPE matches are suppressed against
// feed matches by identity (vulnerability ID or alias), not just by exact ID.
// This is what lets a CGA feed record deduplicate an NVD CPE match that is
// keyed by the CGA's CVE alias.
func Test_deduplicateMatches(t *testing.T) {
	tests := []struct {
		name        string
		feedMatches []match.Match
		cpeMatches  []match.Match
		want        []match.Match
	}{
		{
			name:        "cpe CVE suppressed by feed CGA alias",
			feedMatches: []match.Match{apkMatch("CGA-7qjw-ggh3-pp9f", "CVE-2023-45803")},
			cpeMatches:  []match.Match{apkMatch("CVE-2023-45803")},
			want:        nil,
		},
		{
			name:        "cpe CGA alias suppressed by feed CVE",
			feedMatches: []match.Match{apkMatch("CVE-2023-45803")},
			cpeMatches:  []match.Match{apkMatch("CGA-7qjw-ggh3-pp9f", "CVE-2023-45803")},
			want:        nil,
		},
		{
			name:        "cpe suppressed by exact id match",
			feedMatches: []match.Match{apkMatch("CVE-2023-45803")},
			cpeMatches:  []match.Match{apkMatch("CVE-2023-45803")},
			want:        nil,
		},
		{
			name:        "cpe kept when no id or alias overlap",
			feedMatches: []match.Match{apkMatch("CGA-7qjw-ggh3-pp9f", "CVE-2023-45803")},
			cpeMatches:  []match.Match{apkMatch("CVE-2024-0727")},
			want:        []match.Match{apkMatch("CVE-2024-0727")},
		},
		{
			name:        "only overlapping cpe removed from a mixed set",
			feedMatches: []match.Match{apkMatch("CGA-7qjw-ggh3-pp9f", "CVE-2023-45803")},
			cpeMatches:  []match.Match{apkMatch("CVE-2023-45803"), apkMatch("CVE-2024-0727")},
			want:        []match.Match{apkMatch("CVE-2024-0727")},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := deduplicateMatches(tt.feedMatches, tt.cpeMatches)

			// deduplicateMatches iterates a map, so order is not guaranteed
			sortByID := cmpopts.SortSlices(func(a, b match.Match) bool {
				return a.Vulnerability.ID < b.Vulnerability.ID
			})
			allowUnexported := cmp.Exporter(func(reflect.Type) bool { return true })
			if diff := cmp.Diff(tt.want, got, sortByID, cmpopts.EquateEmpty(), allowUnexported); diff != "" {
				t.Errorf("deduplicateMatches() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}

// failingProvider is a vulnerability.Provider whose FindVulnerabilities always errors, used to
// exercise the error path of vulnSetForPackage. The embedded nil interface is never invoked because
// vulnSetForPackage only calls FindVulnerabilities.
type failingProvider struct {
	vulnerability.Provider
}

func (failingProvider) FindVulnerabilities(...vulnerability.Criteria) ([]vulnerability.Vulnerability, error) {
	return nil, errors.New("boom")
}

// Test_vulnSetForPackage verifies that vulnSetForPackage collects the security feed vulnerabilities
// for the package and for each of its upstream/origin packages, unions them into a single result.Set
// keyed by vulnerability ID, and preserves each record's aliases (RelatedVulnerabilities) so the set
// can later reconcile CPE matches by identity.
func Test_vulnSetForPackage(t *testing.T) {
	const alpineNamespace = "alpine:distro:alpine:3.18"

	directVuln := vulnerability.Vulnerability{
		Reference:   vulnerability.Reference{ID: "CVE-2024-0727", Namespace: alpineNamespace},
		PackageName: "openssl",
		Constraint:  version.MustGetConstraint("< 3.1.4-r5", version.ApkFormat),
	}
	// a feed record for the upstream/source package that carries a CVE alias (as a CGA would)
	upstreamVuln := vulnerability.Vulnerability{
		Reference:              vulnerability.Reference{ID: "CGA-7qjw-ggh3-pp9f", Namespace: alpineNamespace},
		PackageName:            "openssl-src",
		Constraint:             version.MustGetConstraint("< 3.1.4-r5", version.ApkFormat),
		RelatedVulnerabilities: []vulnerability.Reference{{ID: "CVE-2023-45803"}},
	}
	// belongs to a different package and must never be returned
	otherPackageVuln := vulnerability.Vulnerability{
		Reference:   vulnerability.Reference{ID: "CVE-2222-2222", Namespace: alpineNamespace},
		PackageName: "not-openssl",
		Constraint:  version.MustGetConstraint("< 9.9.9", version.ApkFormat),
	}

	withUpstream := dbtest.NewPackage("openssl", "3.1.4-r0", syftPkg.ApkPkg).
		WithDistro(dbtest.Alpine318).
		WithUpstream("openssl-src", "3.1.4-r0").
		Build()
	noUpstream := dbtest.NewPackage("openssl", "3.1.4-r0", syftPkg.ApkPkg).
		WithDistro(dbtest.Alpine318).
		Build()
	...
	tests := []struct {
		...
		pkg      pkg.Package
		want     []vulnerability.Vulnerability
		wantErr  bool
	}{
		{
			name:     "unions direct and upstream feed vulnerabilities",
			provider: mock.VulnerabilityProvider(directVuln, upstreamVuln, otherPackageVuln),
			pkg:      withUpstream,
			want:     []vulnerability.Vulnerability{directVuln, upstreamVuln},
		},
		{
			name:     "returns only the direct vulnerabilities when there is no upstream",
			provider: mock.VulnerabilityProvider(directVuln, upstreamVuln, otherPackageVuln),
			pkg:      noUpstream,
			want:     []vulnerability.Vulnerability{directVuln},
		},
		{
			name:     "propagates provider errors",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := vulnSetForPackage(tt.provider, tt.pkg)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
			...
			// compare the flattened vulnerabilities; the aliases (RelatedVulnerabilities) are part of
			// the vulnerability object, so comparing full objects also asserts they survive.
			sortByID := cmpopts.SortSlices(func(a, b vulnerability.Vulnerability) bool {
				return a.ID < b.ID
			})
			allowUnexported := cmp.Exporter(func(reflect.Type) bool { return true })
			if diff := cmp.Diff(tt.want, got.Vulnerabilities(), sortByID, cmpopts.EquateEmpty(), allowUnexported); diff != "" {
				t.Errorf("vulnSetForPackage() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}

// apkMatchWithFix is apkMatch with a concrete fixed-state fix set, used to assert whether
// cpeMatchesNotResolvedByFeed strips or preserves the NVD fix information.
func apkMatchWithFix(id string, fixVersions []string, aliases ...string) match.Match {
	m := apkMatch(id, aliases...)
	m.Vulnerability.Fix = vulnerability.Fix{
...
// feedVuln builds a distro-feed vulnerability with a vulnerable-version constraint, a fix, and any
// aliases (as a CGA record carries its CVE alias).
func feedVuln(id, constraint string, fixVersions []string, aliases ...string) vulnerability.Vulnerability {
	var related []vulnerability.Reference
	for _, a := range aliases {
		related = append(related, vulnerability.Reference{ID: a})
	}
	return vulnerability.Vulnerability{
		Reference:              vulnerability.Reference{ID: id},
		Constraint:             version.MustGetConstraint(constraint, version.ApkFormat),
		Fix:                    vulnerability.Fix{State: vulnerability.FixStateFixed, Versions: fixVersions},
		RelatedVulnerabilities: related,
	}
}

// Test_cpeMatchesNotResolvedByFeed verifies the keep/drop decision and fix-stripping for CPE (NVD)
// matches given the distro feed: matches are kept when the feed has no record of the vulnerability
// (fix reset to unknown) or the feed still considers the package vulnerable, and dropped only when
// the feed considers the package fixed. Reconciliation is by identity (vulnerability ID or alias).
func Test_cpeMatchesNotResolvedByFeed(t *testing.T) {
	vulnerableVer := version.New("3.1.4-r0", version.ApkFormat) // satisfies "< 3.1.4-r5"
	fixedVer := version.New("3.1.4-r5", version.ApkFormat)      // does not satisfy "< 3.1.4-r5"

	// expected output of case 1 for the match that carried a fix: fix reset to unknown
	cpeFixReset := apkMatch("CVE-2024-0727")
	cpeFixReset.Vulnerability.Fix = vulnerability.Fix{State: vulnerability.FixStateUnknown}

	tests := []struct {
		name      string
		feedVulns []vulnerability.Vulnerability
		cpe       []match.Match
		ver       *version.Version
		want      []match.Match
		wantErr   bool
		...
		{
			name:      "not in feed keeps matches and resets fix to unknown",
			feedVulns: []vulnerability.Vulnerability{feedVuln("CVE-9999-0000", "< 1.0.0", []string{"1.0.0"})},
			cpe:       []match.Match{apkMatchWithFix("CVE-2024-0727", []string{"3.1.4-r5"}), apkMatch("CVE-2024-1111")},
			ver:       vulnerableVer,
			want:      []match.Match{cpeFixReset, apkMatch("CVE-2024-1111")},
		},
		{
			name:      "in feed via alias and still vulnerable keeps all matches with fix intact",
			feedVulns: []vulnerability.Vulnerability{feedVuln("CGA-7qjw-ggh3-pp9f", "< 3.1.4-r5", []string{"3.1.4-r5"}, "CVE-2023-45803")},
			cpe:       []match.Match{apkMatchWithFix("CVE-2023-45803", []string{"9.9.9"})},
			ver:       vulnerableVer,
			want:      []match.Match{apkMatchWithFix("CVE-2023-45803", []string{"9.9.9"})},
		},
		{
			name:      "in feed via alias and fixed drops matches",
			feedVulns: []vulnerability.Vulnerability{feedVuln("CGA-7qjw-ggh3-pp9f", "< 3.1.4-r5", []string{"3.1.4-r5"}, "CVE-2023-45803")},
			cpe:       []match.Match{apkMatch("CVE-2023-45803")},
			ver:       fixedVer,
			want:      nil,
		},
		{
			name:      "in feed by exact id and fixed drops matches",
			feedVulns: []vulnerability.Vulnerability{feedVuln("CVE-2024-0727", "< 3.1.4-r5", []string{"3.1.4-r5"})},
			cpe:       []match.Match{apkMatchWithFix("CVE-2024-0727", []string{"3.1.4-r5"})},
			ver:       fixedVer,
			want:      nil,
		},
		{
			name:      "feed record without fix versions drops matches",
			feedVulns: []vulnerability.Vulnerability{feedVuln("CVE-2024-0727", "< 3.1.4-r5", nil)},
			cpe:       []match.Match{apkMatch("CVE-2024-0727")},
			ver:       vulnerableVer,
			want:      nil,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := cpeMatchesNotResolvedByFeed(newResultSet(tt.feedVulns...), tt.cpe, tt.ver)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
			...
			allowUnexported := cmp.Exporter(func(reflect.Type) bool { return true })
			if diff := cmp.Diff(tt.want, got, sortByID, cmpopts.EquateEmpty(), allowUnexported); diff != "" {
				t.Errorf("cpeMatchesNotResolvedByFeed() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
