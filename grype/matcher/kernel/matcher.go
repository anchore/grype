...
func (m *Matcher) Match(store vulnerability.Provider, p pkg.Package) ([]match.Match, []match.IgnoreFilter, error) {
	...
	if hasReliableKernelData(p.Distro) {
		...
		exactMatches, _, err := internal.MatchPackageByDistro(store, p, nil, m.Type(), nil)
		if err != nil {
...
func (m *Matcher) matchUpstreamPackages(store vulnerability.Provider, p pkg.Package) ([]match.Match, error) {
	...
	for _, indirectPackage := range pkg.UpstreamPackages(p) {
		indirectMatches, _, err := internal.MatchPackageByDistro(store, indirectPackage, &p, m.Type(), nil)
		if err != nil {
