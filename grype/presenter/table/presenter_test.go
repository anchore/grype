...
func TestCreateRow(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			match:           matchWithKev,
			extraAnnotation: appendSuppressed,
			expectedRow:     []string{match1.Artifact.Name, match1.Artifact.Version, "1.0.2, *2.0.1, 3.0.4", string(match1.Artifact.Type), match1.Vulnerability.ID, "Medium", "50.00%", " 87.2", "(has-exploit, suppressed)"},
		},
	}
...
func TestRowsRender(t *testing.T) {
	...
	t.Run("deduplicates identical rows", func(t *testing.T) {
		...
		expected := [][]string{
			{"pkg1", "1.0.0", "1.1.0", "os", "CVE-2023-1234", "critical", "75.00%", "    -"},
		}
	...
	t.Run("renders won't fix and empty fix versions correctly", func(t *testing.T) {
		...
		expected := [][]string{
			{"pkgA", "1.0.0", "", "os", "CVE-2023-1234", "critical", "75.00%", "    -"},
			{"pkgB", "2.0.0", "(won't fix)", "os", "CVE-2023-5678", "high", "75.00%", "    -"},
			{"pkgC", "3.0.0", "3.1.0", "os", "CVE-2023-9012", "medium", "75.00%", "    -"},
		}
