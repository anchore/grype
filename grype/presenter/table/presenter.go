...
func NewPresenter(pb models.PresenterConfig, showSuppressed bool) *Presenter {
	...
	return &Presenter{
		...
		auxiliaryStyle: lipgloss.NewStyle().Foreground(lipgloss.Color("244")).Faint(true), // medium gray
...
func (p *Presenter) newRow(m models.Match, extraAnnotation string, showDistro bool) row {
	...
	if len(m.Vulnerability.KnownExploited) > 0 {
		if p.withColor {
			kev = p.kevStyle.Reverse(false).Render("") + p.kevStyle.Render("Has Exploit") + p.kevStyle.Reverse(false).Render("") // ⚡❋◆◉፨⿻⨳✖•
		} else {
			annotations = append([]string{"has-exploit"}, annotations...)
		}
	}

	// if len(m.Vulnerability.KnownExploited) > 0 {
	//	annotations = append([]string{p.kevStyle.Render("has exploit")}, annotations...)
	//}
