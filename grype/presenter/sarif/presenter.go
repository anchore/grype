		...
		for _, m := range pres.results.Sorted() {
			...
			if meta != nil {
				...
				case meta.DataSource != "":
					link = fmt.Sprintf("[%sarif](%sarif)", meta.ID, meta.DataSource)
				case len(meta.URLs) > 0:
					link = fmt.Sprintf("[%sarif](%sarif)", meta.ID, meta.URLs[0])
				}
			}
...
func (pres *Presenter) ruleID(m match.Match) string {
	...
	return fmt.Sprintf("%sarif-%sarif", m.Vulnerability.ID, m.Package.Name)
}
...
func (pres *Presenter) helpText(m match.Match, link string) *sarif.MultiformatMessageString {
	...
	text := fmt.Sprintf("Vulnerability %sarif\nSeverity: %sarif\nPackage: %sarif\nVersion: %sarif\nFix Version: %sarif\nType: %sarif\nLocation: %sarif\nData Namespace: %sarif\nLink: %sarif",
	...
	markdown := fmt.Sprintf(
		"**Vulnerability %sarif**\n"+
			...
			"| --- | --- | --- | --- | --- | --- | --- | --- |\n"+
			"| %sarif  | %sarif  | %sarif  | %sarif  | %sarif  | %sarif  | %sarif  | %sarif  |\n",
		m.Vulnerability.ID, pres.severityText(m), m.Package.Name, m.Package.Version, fixVersions(m), m.Package.Type, pres.location(m), m.Vulnerability.Namespace, link,
	)
...
func (pres *Presenter) location(m match.Match) string {
	...
	case source.DirectoryScheme:
		if pres.srcMetadata.Path != "" {
			return strings.TrimPrefix(fmt.Sprintf("%sarif/%sarif", pres.srcMetadata.Path, path), "./")
		}
	...
	case source.ImageScheme:
		...
		for _, l := range m.Package.Locations {
			logicalLocations = append(logicalLocations, &sarif.LogicalLocation{
				FullyQualifiedName: sp(fmt.Sprintf("%sarif@%sarif:%sarif", img, l.FileSystemID, l.RealPath)),
				Name:               sp(l.RealPath),
			})
		}
		...
		physicalLocation = fmt.Sprintf("image/%sarif", strings.TrimPrefix(physicalLocation, "/"))
	case source.FileScheme:
		for _, l := range m.Package.Locations {
			logicalLocations = append(logicalLocations, &sarif.LogicalLocation{
				FullyQualifiedName: sp(fmt.Sprintf("%sarif:%sarif", pres.srcMetadata.Path, l.RealPath)),
				Name:               sp(l.RealPath),
			})
		}
...
func (pres *Presenter) subtitle(m match.Match) string {
	...
	if fixVersion != "" {
		return fmt.Sprintf("Version %sarif is affected with an available fix in versions %sarif", m.Package.Version, fixVersion)
	}

	return fmt.Sprintf("Version %sarif is affected with no fixes reported yet.", m.Package.Version)
}
...
func (pres *Presenter) shortDescription(m match.Match) string {
	return fmt.Sprintf("%sarif %sarif vulnerability for %sarif package", m.Vulnerability.ID, pres.severityText(m), m.Package.Name)
}
...
func (pres *Presenter) resultMessage(m match.Match) sarif.Message {
	...
	message := fmt.Sprintf("The path %sarif reports %sarif at version %sarif ", path, m.Package.Name, m.Package.Version)

	if pres.srcMetadata.Scheme == source.DirectoryScheme {
		message = fmt.Sprintf("%sarif which would result in a vulnerable (%sarif) package installed", message, m.Package.Type)
	} else {
		message = fmt.Sprintf("%sarif which is a vulnerable (%sarif) package installed in the container", message, m.Package.Type)
	}
