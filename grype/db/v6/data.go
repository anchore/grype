...
func KnownOperatingSystemSpecifierOverrides() []OperatingSystemSpecifierOverride {
	...
	return []OperatingSystemSpecifierOverride{
		...
		{Alias: "rhel", VersionPattern: `^\d+.\d+`, ReplacementMinorVersion: strRef(""), DBVersionConstraint: "< 6.0.3"},
		// we pass in the distro.Type into the search specifier, not a raw release-id
		{Alias: "redhat", VersionPattern: `^\d+.\d+`, ReplacementMinorVersion: strRef(""), ReplacementName: strRef("rhel"), DBVersionConstraint: "< 6.0.3"},
