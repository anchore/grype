...
func Test_criteriaSetIterator(t *testing.T) {
	name1 := NewPackageNameCriteria("name1")
	name2 := NewPackageNameCriteria("name2")
	name3 := NewPackageNameCriteria("name3")
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, row := range criteriaSetIterator(test.in) {
				got = append(got, row)
			}
