...
import (
	...
	grypeDBv5 "github.com/anchore/grype/internal/db/v5"
	dataV5 "github.com/anchore/grype/internal/db/v5/data"
	grypeDBv6 "github.com/anchore/grype/internal/db/v6"
	dataV6 "github.com/anchore/grype/internal/db/v6/data"
...
func Build(cfg BuildConfig) error {
	...
	for _, sd := range cfg.States {
		sdOpeners, count, err := sd.Openers()
		if err != nil {
...
func getProcessors(cfg BuildConfig) ([]data.Processor, error) {
	...
	case grypeDBv5.SchemaVersion:
		return dataV5.Processors(dataV5.NewConfig(dataV5.WithCPEParts(cfg.IncludeCPEParts), dataV5.WithInferNVDFixVersions(cfg.InferNVDFixVersions))), nil
	case grypeDBv6.ModelVersion:
		return dataV6.Processors(dataV6.NewConfig(dataV6.WithCPEParts(cfg.IncludeCPEParts), dataV6.WithInferNVDFixVersions(cfg.InferNVDFixVersions))), nil
	default:
...
func getWriter(schemaVersion int, dataAge time.Time, directory string, states provider.States) (data.Writer, error) {
	...
	case grypeDBv5.SchemaVersion:
		return dataV5.NewWriter(directory, dataAge, states)
	case grypeDBv6.ModelVersion:
		return dataV6.NewWriter(directory, states)
	default:
