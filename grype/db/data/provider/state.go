...
func (sd State) Openers() (<-chan Opener, int64, error) {
	resultPaths := sd.ResultPaths()
	switch sd.Store {
	case "flat-file":
	...
	return nil, 0, fmt.Errorf("unknown store: %q", sd.Store)
}

func (sd State) Count() (int64, error) {
	resultPaths := sd.ResultPaths()
	switch sd.Store {
	case "flat-file":
	...
	return 0, fmt.Errorf("unknown store: %q", sd.Store)
}
