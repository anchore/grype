...
func TestType_IsKnown(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "known distro - Ubuntu",
			typ:   Ubuntu,
			known: true,
		},
		{
			name:  "known distro - ArchLinux",
			typ:   ArchLinux,
			known: true,
		},
		{
			name:  "known distro - Alpine",
			typ:   Alpine,
			known: true,
		},
		{
			name:  "unknown distro - empty string",
			typ:   Type(""),
			known: false,
		},
		{
			name:  "unknown distro - custom type",
			typ:   Type("customlinux"),
			known: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.known, tt.typ.IsKnown())
		})
	}
}
