...
// these tests document defects found while reviewing the oci product identifier
// rework (#3659). they are expected to fail against that change as written.
//
// the matching direction is what makes most of these load-bearing: go-vex calls
// PurlMatches(docProduct, grypeProduct), and every qualifier on the document
// side must be present and byte-equal on grype's side. a qualifier grype omits
// is fatal to the match; an extra one grype adds is free.
