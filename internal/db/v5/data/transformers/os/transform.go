...
import (
	...
	"github.com/anchore/grype/internal/db/v5/pkg/qualifier"
	"github.com/anchore/grype/internal/db/v5/pkg/qualifier/rpmmodularity"
)
...
func Transform(vulnerability unmarshal.OSVulnerability) ([]data.Entry, error) {
	...
	for idx, fixedInEntry := range vulnerability.Vulnerability.FixedIn {
		...
		allVulns = append(allVulns, v5.Vulnerability{
...
func getCvss(entry unmarshal.OSVulnerability) (cvss []v5.Cvss) {
	for _, vendorCvss := range entry.Vulnerability.CVSS {
		cvss = append(cvss, v5.Cvss{
...
func getAdvisories(entry unmarshal.OSVulnerability, idx int) (advisories []v5.Advisory) {
	fixedInEntry := entry.Vulnerability.FixedIn[idx]

	for _, advisory := range fixedInEntry.VendorAdvisory.AdvisorySummary {
		advisories = append(advisories, v5.Advisory{
			ID:   advisory.ID,
			Link: advisory.Link,
		})
	}
...
func getFix(entry unmarshal.OSVulnerability, idx int) v5.Fix {
	fixedInEntry := entry.Vulnerability.FixedIn[idx]
...
func getRelatedVulnerabilities(entry unmarshal.OSVulnerability) (vulns []v5.VulnerabilityReference) {
	// associate related vulnerabilities from the NVD namespace
	if strings.HasPrefix(entry.Vulnerability.Name, "CVE") {
		vulns = append(vulns, v5.VulnerabilityReference{
