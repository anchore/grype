...
func getFix(matches []nvd.CpeMatch, inferNVDFixVersions bool) v5.Fix {
...
func getCvss(cvss ...nvd.CvssSummary) []v5.Cvss {
	var results []v5.Cvss
	for _, c := range cvss {
		results = append(results, v5.Cvss{
			Source:  c.Source,
			Type:    string(c.Type),
			Version: c.Version,
			Vector:  c.Vector,
			Metrics: v5.CvssMetrics{
