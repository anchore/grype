...
func getFix(entry unmarshal.GitHubAdvisory, idx int) v5.Fix {
...
func getRelatedVulnerabilities(entry unmarshal.GitHubAdvisory) []v5.VulnerabilityReference {
	vulns := make([]v5.VulnerabilityReference, len(entry.Advisory.CVE))
	for idx, cve := range entry.Advisory.CVE {
		vulns[idx] = v5.VulnerabilityReference{
			ID:        cve,
			Namespace: "nvd:cpe",
		}
	}
...
func getCvss(entry unmarshal.GitHubAdvisory) (cvss []v5.Cvss) {
