...
func Test_DifferDirectory(t *testing.T) {
	...
	require.Equal(t, "test-fixtures/dbs/base/"+strconv.Itoa(v6.SchemaVersion), baseStatus.Location)

	err = d.SetTargetDB("test-fixtures/dbs/target")
	require.NoError(t, err)

	targetStatus := d.targetCurator.Status()
	require.Equal(t, "test-fixtures/dbs/target/"+strconv.Itoa(v6.SchemaVersion), targetStatus.Location)
}

func TestPresent_Json(t *testing.T) {
	//GIVEN
	diffs := []v6.Diff{
		{v6.DiffAdded, "CVE-1", "nvd", []string{"requests", "vault"}},
		{v6.DiffRemoved, "CVE-2", "nvd", []string{"k8s"}},
		{v6.DiffChanged, "CVE-3", "nvd", []string{}},
	}
	differ := Differ{}
