...
import (
	...
	"github.com/anchore/grype/internal/db/internal/gormadapter"
...
func New(dbFilePath string, overwrite bool) (v6.Store, error) {
...
func (s *store) GetID() (*v6.ID, error) {
...
func (s *store) SetID(id v6.ID) error {
...
func (s *store) GetVulnerability(namespace, id string) ([]v6.Vulnerability, error) {
	...
	var vulnerabilities = make([]v6.Vulnerability, len(models))
...
func (s *store) SearchForVulnerabilities(namespace, packageName string) ([]v6.Vulnerability, error) {
	var models []model.VulnerabilityModel
...
func (s *store) AddVulnerability(vulnerabilities ...v6.Vulnerability) error {
...
func (s *store) GetVulnerabilityMetadata(id, namespace string) (*v6.VulnerabilityMetadata, error) {
...
//
//nolint:gocognit
func (s *store) AddVulnerabilityMetadata(metadata ...v6.VulnerabilityMetadata) error {
...
func (s *store) GetVulnerabilityMatchExclusion(id string) ([]v6.VulnerabilityMatchExclusion, error) {
	var models []model.VulnerabilityMatchExclusionModel

	result := s.db.Where("id = ?", id).Find(&models)

	var exclusions []v6.VulnerabilityMatchExclusion
...
func (s *store) AddVulnerabilityMatchExclusion(exclusions ...v6.VulnerabilityMatchExclusion) error {
...
func (s *store) GetAllVulnerabilities() (*[]v6.Vulnerability, error) {
	var models []model.VulnerabilityModel
	if result := s.db.Find(&models); result.Error != nil {
		return nil, result.Error
	}
	vulns := make([]v6.Vulnerability, len(models))
	for idx, m := range models {
		vuln, err := m.Inflate()
		if err != nil {
			return nil, err
...
func (s *store) GetAllVulnerabilityMetadata() (*[]v6.VulnerabilityMetadata, error) {
	var models []model.VulnerabilityMetadataModel
	if result := s.db.Find(&models); result.Error != nil {
		return nil, result.Error
	}
	metadata := make([]v6.VulnerabilityMetadata, len(models))
	for idx, m := range models {
		data, err := m.Inflate()
		if err != nil {
			return nil, err
...
func (s *store) DiffStore(targetStore v6.StoreReader) (*[]v6.Diff, error) {
