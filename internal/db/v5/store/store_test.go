...
import (
	...
	"github.com/anchore/grype/internal/db/v5/store/model"
)

func assertIDReader(t *testing.T, reader v6.IDReader, expected v6.ID) {
	t.Helper()
...
func assertVulnerabilityReader(t *testing.T, reader v6.VulnerabilityStoreReader, namespace, name string, expected []v6.Vulnerability) {
	...
	extra := []v6.Vulnerability{
		{
			...
			RelatedVulnerabilities: []v6.VulnerabilityReference{
				{
					ID:        "another-cve",
					Namespace: "nvd",
				},
		...
		},
		{
			...
			Fix: v6.Fix{
				State: v6.NotFixedState,
			},
		},
	}

	expected := []v6.Vulnerability{
		{
			ID:                "my-cve",
		...
		},
		{
			...
			Advisories: []v6.Advisory{{ID: "ABC-12345", Link: "https://abc.xyz"}},
		},
	}
...
func assertVulnerabilityMetadataReader(t *testing.T, reader v6.VulnerabilityMetadataStoreReader, id, namespace string, expected v6.VulnerabilityMetadata) {
...
func sortMetadataCvss(cvss []v6.Cvss) {
...
func TestStore_GetVulnerabilityMetadata_SetVulnerabilityMetadata(t *testing.T) {
	...
	total := []v6.VulnerabilityMetadata{
		{
			ID:           "my-cve",
...
func TestStore_MergeVulnerabilityMetadata(t *testing.T) {
	tests := []struct {
		name     string
		add      []v6.VulnerabilityMetadata
		expected v6.VulnerabilityMetadata
		err      bool
	}{
		{
			name: "go-case",
...
func assertVulnerabilityMatchExclusionReader(t *testing.T, reader v6.VulnerabilityMatchExclusionStoreReader, id string, expected []v6.VulnerabilityMatchExclusion) {
...
func TestStore_GetVulnerabilityMatchExclusion_SetVulnerabilityMatchExclusion(t *testing.T) {
	...
	extra := []v6.VulnerabilityMatchExclusion{
		{
			ID: "CVE-1234-14567",
			Constraints: []v6.VulnerabilityMatchExclusionConstraint{
				{
					Vulnerability: v6.VulnerabilityExclusionConstraint{
						Namespace: "extra-namespace:cpe",
					},
					Package: v6.PackageExclusionConstraint{
						Name:     "abc",
						Language: "ruby",
						Version:  "1.2.3",
					},
	...
	expected := []v6.VulnerabilityMatchExclusion{
		{
			ID: "CVE-1234-9999999",
			Constraints: []v6.VulnerabilityMatchExclusionConstraint{
				{
...
func Test_DiffStore(t *testing.T) {
	...
	baseMetadata := []v6.VulnerabilityMetadata{
		{
			Namespace:  "nuget",
			ID:         "GHSA-****-******",
			DataSource: "nvd",
	...
	expectedDiffs := []v6.Diff{
		{
			Reason:    v6.DiffChanged,
			ID:        "CVE-123-4567",
			Namespace: "github:language:python",
