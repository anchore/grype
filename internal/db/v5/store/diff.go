...
type storeVulnerability struct {
	item *v6.Vulnerability
	seen bool
}
type storeMetadata struct {
	item *v6.VulnerabilityMetadata
	seen bool
}
...
func buildVulnerabilityPkgsMap(models *[]v6.Vulnerability) *map[storeKey][]string {
...
func createDiff(baseStore, targetStore *PkgMap, key storeKey, reason v6.DiffReason) *v6.Diff {
...
func getVulnerabilityParentKey(vuln v6.Vulnerability) storeKey {
	return storeKey{vuln.ID, vuln.Namespace, ""}
}

// gets a packaged key from a vulnerability
func getVulnerabilityKey(vuln v6.Vulnerability) storeKey {
...
func NewVulnerabilitySet(models *[]v6.Vulnerability) *VulnerabilitySet {
...
func (v *VulnerabilitySet) in(item v6.Vulnerability) bool {
	_, exists := v.data[getVulnerabilityParentKey(item)]
	return exists
}

func (v *VulnerabilitySet) match(item v6.Vulnerability) bool {
...
func diffVulnerabilities(baseModels, targetModels *[]v6.Vulnerability, basePkgsMap, targetPkgsMap *PkgMap, differentItems *progress.Manual) *map[string]*v6.Diff {
	diffs := make(map[string]*v6.Diff)
	...
	for _, tModel := range *targetModels {
		...
		if m.in(targetModel) {
			...
			if !matched {
				...
				diffs[k.id+k.namespace] = createDiff(basePkgsMap, targetPkgsMap, k, v6.DiffChanged)
			...
			diffs[k.id+k.namespace] = createDiff(nil, targetPkgsMap, k, v6.DiffAdded)
			differentItems.Increment()
		}
	}
	...
	for _, k := range notSeen {
		...
		diffs[k.id+k.namespace] = createDiff(basePkgsMap, nil, k, v6.DiffRemoved)
		differentItems.Increment()
	}
...
func NewMetadataSet(models *[]v6.VulnerabilityMetadata) *MetadataSet {
...
func (v *MetadataSet) in(item v6.VulnerabilityMetadata) bool {
	_, exists := v.data[getMetadataKey(item)]
	return exists
}

func (v *MetadataSet) match(item v6.VulnerabilityMetadata) bool {
...
func diffVulnerabilityMetadata(baseModels, targetModels *[]v6.VulnerabilityMetadata, basePkgsMap, targetPkgsMap *PkgMap, differentItems *progress.Manual) *map[string]*v6.Diff {
	diffs := make(map[string]*v6.Diff)
...
func getMetadataKey(metadata v6.VulnerabilityMetadata) storeKey {
