...
import (
	...
	v6Dist "github.com/anchore/grype/internal/db/v6/distribution"
...
func writeLatestDocument(tarPath string, metadata v6.DBMetadata) error {
	archive, err := v6Dist.NewArchive(tarPath, *metadata.BuildTimestamp, metadata.Model, metadata.Revision, metadata.Addition)
	if err != nil || archive == nil {
		return fmt.Errorf("unable to create archive: %w", err)
	}

	doc := v6Dist.NewLatestDocument(*archive)
	if doc == nil {
	...
	latestPath := path.Join(dbDir, v6Dist.LatestFileName)
