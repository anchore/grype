...
func (c Config) DBFilePath() string {
	return filepath.Join(c.DBDirectoryPath(), v6.VulnerabilityDBFileName)
}

func (c Config) DBDirectoryPath() string {
	return filepath.Join(c.DBRootDir, strconv.Itoa(v6.ModelVersion))
}
...
func NewCurator(cfg Config, downloader distribution.Client) (v6.Curator, error) {
	return curator{
		fs:       afero.NewOsFs(),
		client:   downloader,
		config:   cfg,
		hydrator: v6.Hydrater(),
	}, nil
}

func (c curator) Reader() (v6.Reader, error) {
	s, err := v6.NewReader(
		v6.Config{
	...
	doRehydrate, err := isRehydrationNeeded(c.fs, c.config.DBDirectoryPath(), currentDBSchemaVersion, schemaver.New(v6.ModelVersion, v6.Revision, v6.Addition))
	if err != nil {
		return nil, err
	}
	if doRehydrate {
		...
		var source string
		im, err := v6.ReadImportMetadata(c.fs, c.config.DBDirectoryPath())
		if err == nil && im != nil {
...
func (c curator) Status() vulnerability.ProviderStatus {
	...
	d, validateErr := v6.ReadDescription(dbFile)
	if validateErr != nil {
	...
	var source string
	im, readErr := v6.ReadImportMetadata(c.fs, c.config.DBDirectoryPath())
...
func (c curator) Update() (bool, error) {
	current, err := v6.ReadDescription(c.config.DBFilePath())
	if err != nil {
...
func (c curator) update(current *v6.Description) (*distribution.Archive, error) {
	mon := newMonitor()
...
func isRehydrationNeeded(fs afero.Fs, dirPath string, currentDBVersion *schemaver.SchemaVer, currentClientVersion schemaver.SchemaVer) (bool, error) {
	...
	importMetadata, err := v6.ReadImportMetadata(fs, dirPath)
	if err != nil {
...
func (c curator) Import(reference string) error {
	...
	if isURL(reference) {
		...
		var err error
		tempDir, err = os.MkdirTemp(c.config.DBRootDir, fmt.Sprintf("tmp-v%v-import", v6.ModelVersion))
		if err != nil {
		...
		if strings.HasSuffix(reference, ".db") {
			...
			if err := file.CopyFile(afero.NewOsFs(), reference, filepath.Join(tempDir, v6.VulnerabilityDBFileName)); err != nil {
...
func (c curator) hydrate(dbDirPath, from string, mon monitor) error {
	...
	doc, err := v6.WriteImportMetadata(c.fs, dbDirPath, from)
	if err != nil {
...
func (c curator) validateIntegrity(description *v6.Description) (string, error) {
	...
	if description.SchemaVersion.Model != v6.ModelVersion {
		return "", fmt.Errorf("unsupported database version: have=%d want=%d", description.SchemaVersion.Model, v6.ModelVersion)
	}
	...
	importMetadata, err := v6.ReadImportMetadata(c.fs, filepath.Dir(dbFilePath))
	if err != nil {
		return "", err
	}
...
func (c curator) validateAge(m *v6.Description) error {
	if m == nil {
