...
func (m *mockClient) IsUpdateAvailable(current *v6.Description) (*distribution.Archive, error) {
...
func setupCuratorForUpdate(t *testing.T, opts ...setupOption) curator {
	...
	oldDescription := v6.Description{
		SchemaVersion: schemaver.New(v6.ModelVersion, v6.Revision, v6.Addition),
		Built:         v6.Time{Time: time.Now().Add(-48 * time.Hour)},
	}
	writeTestDB(t, c.fs, dbDir)

	newDescription := oldDescription
	newDescription.Built = v6.Time{Time: time.Now()}
...
func writeTestDescriptionToDB(t *testing.T, dir string, desc v6.Description) string {
	c := v6.Config{DBDirPath: dir}
	d, err := v6.NewLowLevelDB(c.DBFilePath(), false, true, true)
	require.NoError(t, err)

	if err := d.Where("true").Delete(&v6.DBMetadata{}).Error; err != nil {
	...
	digest, err := v6.CalculateDBDigest(afero.NewOsFs(), c.DBFilePath())
	require.NoError(t, err)
...
func writeTestImportMetadata(t *testing.T, fs afero.Fs, dir string, checksums string) {
	writeTestImportMetadataWithCustomVersion(t, fs, dir, checksums, schemaver.New(v6.ModelVersion, v6.Revision, v6.Addition).String())
}

func writeTestImportMetadataWithCustomVersion(t *testing.T, fs afero.Fs, dir string, checksums string, ver string) {
	require.NoError(t, fs.MkdirAll(dir, 0755))

	metadataFilePath := filepath.Join(dir, v6.ImportMetadataFileName)
...
func writeTestDB(t *testing.T, fs afero.Fs, dir string) string {
	...
	doc, err := v6.WriteImportMetadata(fs, dir, "source")
	require.NoError(t, err)
...
func TestCurator_validateAge(t *testing.T) {
	...
	hoursAgo := func(h int) v6.Time {
		return v6.Time{Time: time.Now().UTC().Add(-time.Duration(h) * time.Hour)}
	}
...
func TestCurator_validateIntegrity(t *testing.T) {
	newCurator := func(t *testing.T) (curator, *v6.Description) {
		tempDir := t.TempDir()
		cfg := testConfig()
		cfg.DBRootDir = tempDir
		...
		digest, err := v6.CalculateDBDigest(afero.NewOsFs(), cfg.DBFilePath())
		require.NoError(t, err)
		...
		return ci.(curator), v6.DescriptionFromMetadata(m)
	}
	...
	t.Run("import metadata file does not exist", func(t *testing.T) {
		c, d := newCurator(t)
		dbDir := c.config.DBDirectoryPath()
		require.NoError(t, os.Remove(filepath.Join(dbDir, v6.ImportMetadataFileName)))
		_, err := c.validateIntegrity(d)
	...
	t.Run("unsupported database version", func(t *testing.T) {
		...
		d.SchemaVersion = schemaver.New(v6.ModelVersion-1, 0, 0)
...
func setupTestDB(t *testing.T, dbDir string) v6.ReadWriter {
	s, err := v6.NewWriter(v6.Config{
		DBDirPath: dbDir,
	})
	require.NoError(t, err)
...
func setupReadOnlyTestDB(t *testing.T, dbDir string) v6.Reader {
	s, err := v6.NewReader(v6.Config{
		DBDirPath: dbDir,
	})
	require.NoError(t, err)
