...
import (
	...
	distroNs "github.com/anchore/grype/internal/db/v5/namespace/distro"
	"github.com/anchore/grype/internal/db/v5/namespace/language"
	v7 "github.com/anchore/grype/internal/db/v6"
...
func (s *ServerBuilder) buildDB() []byte {
	...
	for _, v := range s.Vulnerabilities {
		...
		var operatingSystem *v7.OperatingSystem
		packageType := ""
		...
		if d != nil {
			...
			operatingSystem = &v7.OperatingSystem{
				Name:         d.Provider(),
		...
		vuln := &v7.VulnerabilityHandle{
			...
			BlobValue: &v7.VulnerabilityBlob{
				...
				Severities: []v7.Severity{
					{
						Scheme: v7.SeveritySchemeCVSS,
						Value:  "high",
						Source: "",
						Rank:   0,
					},
		...
		if prov.ID != "nvd" {
			...
			for _, c := range cpes {
				ac := &v7.AffectedCPEHandle{
					Vulnerability: vuln,
					CPE:           &c,
					BlobValue: &v7.AffectedPackageBlob{
						Ranges: []v7.AffectedRange{
							{
...
func toAffectedVersion(c version.Constraint) v7.AffectedVersion {
	parts := strings.SplitN(c.String(), "(", 2)
	if len(parts) < 2 {
		return v7.AffectedVersion{
