...
type Client interface {
	...
	IsUpdateAvailable(current *v7.Description) (*Archive, error)
...
func (c client) IsUpdateAvailable(current *v7.Description) (*Archive, error) {
...
func (c client) isUpdateAvailable(current *v7.Description, candidate *LatestDocument) (*Archive, string) {
	if candidate == nil {
		return nil, ""
	}
...
func (c client) latestURL() string {
	...
	if !strings.HasSuffix(u, ".json") {
		...
		u = fmt.Sprintf("%s/v%d/%s", u, v7.ModelVersion, LatestFileName)
	}
	return u
}
...
func isSupersededBy(current *v7.Description, candidate v7.Description) bool {
	if current == nil {
