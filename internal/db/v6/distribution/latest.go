...
func NewLatestDocument(entries ...Archive) *LatestDocument {
	...
	for _, entry := range entries {
		if entry.SchemaVersion.Model == v6.ModelVersion {
...
func calculateArchiveDigest(dbFilePath string) (string, error) {
	digest, err := file.ContentDigest(afero.NewOsFs(), dbFilePath, sha256.New())
	if err != nil {
