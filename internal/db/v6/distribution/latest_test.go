	...
	t.Run("valid entries", func(t *testing.T) {
		archive1 := Archive{
			Description: v6.Description{
				...
				Built:         v6.Time{Time: time.Now()},
			},
		}
		archive2 := Archive{
			Description: v6.Description{
				SchemaVersion: schemaver.New(v6.ModelVersion, v6.Revision, v6.Addition),
				Built:         v6.Time{Time: time.Now().Add(-1 * time.Hour)},
			},
		}
		...
		require.Equal(t, latestDoc.SchemaVersion.Model, v6.ModelVersion)
	})

	t.Run("filter entries", func(t *testing.T) {
		archive1 := Archive{
			Description: v6.Description{
				SchemaVersion: schemaver.New(5, v6.Revision, v6.Addition), // old!
				Built:         v6.Time{Time: time.Now()},
			},
		}
		archive2 := Archive{
	...
	t.Run("valid JSON", func(t *testing.T) {
		latestDoc := LatestDocument{
			Archive: Archive{
				Description: v6.Description{
					SchemaVersion: schemaver.New(v6.ModelVersion, v6.Revision, v6.Addition),
					Built:         v6.Time{Time: time.Now().Truncate(time.Second).UTC()},
				},
			},
			Status: "active",
		}
...
func TestLatestDocument_Write(t *testing.T) {
	...
	now := v6.Time{Time: time.Now().Truncate(time.Second).UTC()}
	...
	}{
		{
			...
			expected: &Archive{
				Description: v6.Description{
					...
					Built:         v6.Time{Time: time.Date(2023, 11, 24, 12, 0, 0, 0, time.UTC)},
				},
				Path:     "archive.tar.gz",
