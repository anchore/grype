...
type FixChannel struct {
	...
	Versions string                   `yaml:"versions" json:"versions" mapstructure:"versions"` // the version constraint that this channel applies to, e.g. ">= 8.0"
}
...
func DefaultFixChannels() FixChannels {
	...
	rhelEus := FixChannel{
		...
		Versions: ">= 8.0", // this is the minimum version that EUS applies to, as of the time of writing
	}

	// use the API defaults
	apiDefaults := distro.DefaultFixChannels()
	for _, channel := range apiDefaults {
		if strings.ToLower(channel.Name) == "eus" {
			rhelEus.Apply = channel.Apply
			rhelEus.IDs = channel.IDs
			rhelEus.Versions = channel.Versions.Value()
			break
		}
	}
