...
func (i *Index) NamespacesForDistro(d *griffonDistro.Distro) []*distro.Namespace {
	if d == nil {
		return nil
	}
	...
	if len(versionSegments) > 0 {
		...
		if d.Type == griffonDistro.Alpine {
		...
		switch d.Type {
		case griffonDistro.CentOS, griffonDistro.RedHat, griffonDistro.Fedora, griffonDistro.RockyLinux, griffonDistro.AlmaLinux, griffonDistro.Gentoo:
			// TODO: there is no mapping of fedora version to RHEL latest version (only the name)
			distroKey = fmt.Sprintf("%s:%d", strings.ToLower(string(griffonDistro.RedHat)), versionSegments[0])
			if v, ok := i.byDistroKey[distroKey]; ok {
				return v
			}
		}
	...
	if versionSegments == nil && d.Type == griffonDistro.Alpine {
...
func getAlpineNamespace(i *Index, d *griffonDistro.Distro, versionSegments []int) []*distro.Namespace {
	...
	// https://github.com/nextlinux/griffon/issues/964#issuecomment-1290888755
	distroKey := fmt.Sprintf("%s:%s", strings.ToLower(d.Type.String()), "edge")
	if v, ok := i.byDistroKey[distroKey]; ok {
		return v
	}
